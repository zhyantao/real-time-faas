...
class FunctionCount:
    ...
    action_names = ['hello-python', 'hash-python', 'cryptography-python', 'md5-python', 'sort-python',
                    'facerecognition-general', 'multinomialnb-general',
                    'passiveaggressiveclassifier-general', 'perceptron-general', 'randomforestregressor-general',
                    'sgdclassifier-general', 'sgdregressor-general', 'svr-general', 'bigdata', 'stream']
...
class Prometheus:
    perf_metrics = ['node_perf_cpu_migrations_total', 'node_perf_branch_misses_total',
    ...
    system_level_metrics = ['container_cpu_usage_seconds_total', 'container_memory_rss',
    ...
    def system_level(self, metric_name='container_cpu_usage_seconds_total', start=1629342630, end=1629343530, step=15,
                     namespace='openfaas-fn'):
        url = 'http://serverless.siat.ac.cn:30965/api/datasources/proxy/1/api/v1/query_range?query={metric}&start={start}&end={end}&step={step}'
        metric = 'sum(rate({mtr}'.format(
            mtr=metric_name) + '{namespace=~"' + namespace + '"}[20s])) by (pod) * on(pod) group_right kube_pod_container_info'
