...
if __name__ == '__main__':
    ...
    while idx < rows:
        ...
        for epoch in range(10000):
            ...
            print("y_pred = [" +
                  ", ".join(["% 2.5f"
                             ...
                             for i in range(len(std_y))]) +
                  "]", end=", ")
            loss = lstm_net.y_list_is(std_y, ToyLossLayer)
            print("loss:", "%.3e" % loss)
            # with open(f_loss_path, 'a') as f:
            #     item = pd.DataFrame(loss)
            #     item.to_csv(f, header=False, index=False, line_terminator="\n")
        ...
        # (5) 预测和计算损失
        # print("y_pred = [" +
        #       ", ".join(["% 2.5f" % lstm_net.lstm_node_list[i].state.h[0] for i in range(len(std_y))]) +
        #       "]", end=", ")
        # loss = lstm_net.y_list_is(std_y, ToyLossLayer)
        # print("loss:", "%.3e" % loss)
