...
ROOT_PATH = 'E:/Workshop/real-time-faas/dataset/'
parser = argparse.ArgumentParser(description='REAL_TIME_FAAS')

# -- Dataset --
parser.add_argument('--selected_container_usage_path', type=str, default=ROOT_PATH + 'selected_container_usage.csv',
                    help='从 container_usage.csv 中选出的数据')
parser.add_argument('--container_usage_path', type=str, default=ROOT_PATH + 'container_usage.csv',
                    help='从 container_usage.csv 中选出的数据')
...
parser.add_argument('--batch_task_path', type=str, default=ROOT_PATH + 'batch_task.csv', help='原始数据 batch_task.csv')
parser.add_argument('--selected_batch_task_path', type=str, default=ROOT_PATH + 'selected_batch_task.csv',
                    help='batch_task.csv 采样后的数据')
parser.add_argument('--batch_task_topological_order_path', type=str,
                    default=ROOT_PATH + 'batch_task_topological_order.csv',
                    help='selected_batch_task.csv 拓扑排序后的数据')
parser.add_argument('--batch_instance_path', type=str, default=ROOT_PATH + 'batch_instance.csv',
                    help='原始数据 batch_instance.csv')
parser.add_argument('--selected_batch_instance_path', type=str, default=ROOT_PATH + 'selected_batch_instance.csv',
                    help='batch_instance.csv 采样后的数据')
parser.add_argument('--batch_instance_topological_order_path', type=str,
                    default=ROOT_PATH + 'batch_instance_topological_order.csv',
                    help='selected_batch_instance.csv 拓扑排序后的数据')
parser.add_argument('--result_saving_path', type=str, default=ROOT_PATH + '../results', help='保存实验结果的路径')
