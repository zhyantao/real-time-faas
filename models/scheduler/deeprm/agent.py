...
class CompactScheduler(Scheduler):
    ...
    def schedule(self):
        ...
        for i_task in range(len(self.environment.queue)):
            pairs = [(i_node, self.environment.nodes[i_node].utilization()) for i_node in
                     range(len(self.environment.nodes))]
...
class CNNModel(tf.keras.Model):
    ...
    def __init__(self, input_shape, output_shape):
        ...
        if os.path.isfile('__cache__/model/deeprm.h5'):
            self.model = tf.keras.models.load_model('__cache__/model/deeprm.h5', compile=False)
        else:
