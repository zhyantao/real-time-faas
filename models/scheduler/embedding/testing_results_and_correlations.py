...
# Read in data
df = pd.read_csv("results/final_adjacency_matrix.csv")
...
# Reconstruct the DAG
final_DAG = from_numpy_array(df.to_numpy(), create_using=nx.DiGraph)
...
interactive_plot = netgraph.InteractiveGraph(final_DAG, node_labels=dict(zip(new_vars, list(new_vars))))
plt.show()
...
# Read in the data
df = pd.read_csv("../../../dataset/simulated_data.csv")
