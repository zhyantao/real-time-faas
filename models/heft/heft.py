    ...
    while visit_queue:
        ...
        if metric == RankMetric.MEAN:
            ...
            for succnode in dag.successors(node):
                ...
                logger.debug(
                    f"\tThe edge weight from node {node} to node {succnode} is {dag[node][succnode]['avgweight']}, and the ranku for node {node} is {dag.nodes()[succnode]['ranku']}")
            ...
            logger.debug(
                f"\tNode {node} has maximum computation cost of {comp_matrix_masked[node - _self.numExistingJobs][max_node_idx]} on processor {max_node_idx}")
            for succnode in dag.successors(node):
                logger.debug(f"\tLooking at successor node: {succnode}")
                ...
                logger.debug(
                    f"\tNode {succnode} has maximum computation cost of {comp_matrix_masked[succnode - _self.numExistingJobs][max_succ_idx]} on processor {max_succ_idx}")
...
def _node_can_be_processed(_self, dag, node):
    ...
    for succnode in dag.successors(node):
        if 'ranku' not in dag.nodes()[succnode]:
            logger.debug(
                f"Attempted to compute the Rank U for node {node} but found that it has an unprocessed successor {dag.nodes()[succnode]}. Will try with the next node in the queue")
            return False
    return True
...
def _compute_eft(_self, dag, node, proc):
    ...
    for idx in range(len(job_list)):
        ...
        logger.debug(
            f"\tLooking to fit a job of length {computation_time} into a slot of size {next_job.start - max(ready_time, prev_job.end)}")
        if (next_job.start - computation_time) - max(ready_time, prev_job.end) >= 0:
            job_start = max(ready_time, prev_job.end)
            logger.debug(
                f"\tInsertion is feasible. Inserting job with start time {job_start} and end time {job_start + computation_time} into the time slot [{prev_job.end}, {next_job.start}]")
            min_schedule = ScheduleEvent(node, job_start, job_start + computation_time, proc)
            break
    else:
...
if __name__ == "__main__":
    ...
    if (communication_matrix.shape[0] != communication_matrix.shape[1]):
        assert communication_matrix.shape[0] - 1 == communication_matrix.shape[
            1], "If the communication_matrix CSV is non-square, there must only be a single additional row specifying the communication startup costs of each PE"
        logger.debug(
